...
NUM=0                            # id of nation (0 for globe)
############################################################
# Geography (Edit here if you change spatial domain/resolution)
############################################################
L="9331200"                          # total num of cells
XY="4320 2160"                        # X and Y
...
FWITIND=../../map/dat/wit_ind_/AQUASTAT20000000${SUF}
FWITDOM=../../map/dat/wit_dom_/AQUASTAT20000000${SUF}
#
...
if [ -f $FRIVOUT ]; then
  ...
  #htmask $ARG $TEMP  $FFLWDIR eq 9   $TEMP > $DN # @original
  htmask $ARG $TEMP $FRIVMOU eq 9 $TEMP > $DN # @kajiyama
...
############################################################
FENVFLW=$DIRENVOUT/$PRJENV$RUNENV$YEAR$MON$DAY$SUF
if [ -f $FENVFLW ]; then
